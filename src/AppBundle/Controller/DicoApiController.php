...
class DicoApiController extends Controller
{
  ...
  */
  public function contraire($mot,$nb)
  {
    $em = $this->container->get('neo4j.manager');
    $user_neo = $this->container->getParameter('neo4j_user');
    $pass_neo = $this->container->getParameter('neo4j_pass');
  ...
  */
  public function has_part($mot,$nb)
  {
    $em = $this->container->get('neo4j.manager');
    $user_neo = $this->container->getParameter('neo4j_user');
    $pass_neo = $this->container->getParameter('neo4j_pass');
  ...
  */
  ...
  /**
  * @Route("/search/{mot}",name="autocompletion")
  **/
  public function searchAuto($mot)
  {
    $em = $this->container->get('neo4j.manager');
    $user_neo = $this->container->getParameter('neo4j_user');
    $pass_neo = $this->container->getParameter('neo4j_pass');
    ...
    ->addConnection('default',$login)
    ->build();
    $query = sprintf('MATCH (n:termes) where n.name STARTS WITH "%s" return n order by n.name skip 1 limit 5',$mot);
    $result = $client->run($query);
    $arr = array();
    foreach ($result->records() as $record) {
